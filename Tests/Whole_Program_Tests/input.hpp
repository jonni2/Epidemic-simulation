...
// the folder
inline void default_input_file(std::string settings_path, std::string input_board_path) {
  std::ofstream os;
  os.open(settings_path);

  os << "base 70\n";
  os << "height 20\n\n";
  os << "quarantine_threshold 0.05\n\n";
  os << "heal_threshold 2.5\n";
  os << "infect_threshold 0.069\n";
  os << "death_threshold 0.75";
  os.close();

  os.open(input_board_path);
...
// by the program
inline void check_input_file(std::string settings_path, std::string input_board_path) {
  std::ifstream is;
  ...
  if (count_lines != height + 2) {
    ...
    std::cout << "\u001b[31;1m" << "\n(!)" << "\u001b[37;1m" << " The previous input board was not correct and has been re-created\n";
    std::ofstream os;
    os.open(input_board_path);
...
inline Input_Values read_input(std::string settings_path) {
...
inline void Fill_Board_From_Input(Board &board, std::string settings_path, std::string input_board_path) {
